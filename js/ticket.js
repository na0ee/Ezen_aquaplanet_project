...
// 드롭다운 밖(외부 화면)을 클릭하면 열려 있는 할인 드롭다운을 닫는다
document.addEventListener('click', (e) => {
  if (e.target.closest('.ticket-discount')) return; // 드롭다운 내부 클릭은 유지
  document.querySelectorAll('.ticket-discount[open]').forEach((det) => {
    det.open = false;
  });
});
