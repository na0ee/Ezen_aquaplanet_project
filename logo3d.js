...
const ABBERATION  = 0.15;            // air.inc식 색수차 림 강도 (0=없음, 0.3~1.2 권장)
...
async function initLogo3D() {
  ...
  function updateHoverTarget(event) {
    if (logoDocked || footerActive || !floatAnim) {
      hoverTarget = 0;
      return;
    }
  ...
  loader.loadAsync(SYMBOL_SRC).then((gltf) => {
    ...
    // mesh별 미세 조정은 카메라 fit 이후 적용해, 크기/위치 변화가 카메라 줌아웃에 묻히지 않게 한다.
  ...
  function tick() {
    ...
    if (!active && !footerActive && !needsRender && !shouldFloat && !shouldHover && !STYLE.refract) return;
    ...
    if (!active && !footerActive && !shouldFloat && !shouldHover && !STYLE.refract) needsRender = false;
  }
  tick();
